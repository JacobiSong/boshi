...
var first = true
var users = {}
setInterval("check_chat_history()", "2000000");
...
function send_message(question, user_id, msg_str) {
    /*
     * 发送消息，返回消息发送状态
     * (如果在本地缓存了聊天记录，则需要在这个函数里将新消息添加到聊天记录中！！！！前端代码中没有对应数据结构缓存聊天记录！！！！！！！)
     * @params:
     *   user_id -> string:    当前聊天的用户标识符
...
function add_new_chat(user_id, user_name) {
    ...
    html = '<div class="user-info layui-row" id="' + user_id + '" onclick="choose_chat(this.id)">\n' +
        ...
        '                    <img src="http://boshitech.net/service/resources/img/mine_fill.png">\n' +
        '                </div>\n' +
...
function add_client_msg(user_id, msg) {
    ...
    html += '  <div class="message-client-content message-content">\n' +
        '        <div  onclick="choose_question(this.id)" id="'+msg+'">' + msg + '</div>\n' +
        '    </div>\n' +
        '    </div>\n';
    return html
}

function choose_question(msg){
    console.log("choose a new question")
    console.log(typeof(msg))
    console.log(msg)
    question = msg
    /*
    * 选择新问题后，查询问题及相关答案，显示在前端
    */
...
function choose_chat(user_id) {
    ...
    chats = []
    for(var p in chat_history[current_user_id]){
        chats.push(chat_history[current_user_id][p])
    }
    console.log(chat_history)
	console.log(chats)
	ret_chat = {}
